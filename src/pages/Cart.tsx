...
const Cart = () => {
  ...
  const openDeleteDialog = (variantId: string, productName: string) => {
  ...
  const handleCheckout = () => {
    ...
    if (checkoutUrl) {
      ...
      toast.error("Checkout not ready", {
        description: "Add an item to your cart first."
      });
    }
  };
  ...
  const handleApplyPromo = () => {
    ...
    // Shopify checkout handles promo codes directly; this is just local feedback.
    if (promoCode.toUpperCase() === 'WELCOME10') {
      setActivePromoCode('WELCOME10');
      toast.success(`Promo code ${promoCode.toUpperCase()} will be applied at checkout`);
      setPromoCode('');
    } else {
                  ...
                  <CardContent className="flex-1 p-4 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <Button
                        ...
                        onClick={() => openDeleteDialog(item.variantId, item.product.name)}
                        disabled={isLoading}
                      >
            ...
            <CardContent className="p-6">
              ...
              <div className="mb-4 rounded-lg bg-muted/50 p-3">
                ...
                <div className="h-2 w-full rounded-full bg-background overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-primary to-accent transition-all" style={{ width: `${freeShippingPct}%` }} />
                </div>
              </div>
              ...
              <motion.div whileTap={{ scale: 0.97 }}>
                <Button className="w-full" size="lg" onClick={handleCheckout} disabled={isLoading || items.length === 0}>
                  {isLoading ? (
                    <span className="animate-pulse">Updating cart…</span>
                  ) : (
                    <><ExternalLink className="mr-2 h-4 w-4" /> Checkout with Shopify</>
                  )}
                </Button>
              </motion.div>
              ...
              <div className="mt-4">
                ...
                {activePromoCode && (
                  <p className="text-green-600 text-sm mt-1">
                    {activePromoCode} will be applied at Shopify checkout
                  </p>
                )}
              </div>
            </CardContent>
